...
class MazeSolverApp:
    def __init__(self, root):
        ...
        self.start_pos = None
        self.goal_pos = None
        self.current_tool = OPEN  # Current tool: Start, Goal, or Wall
        ...
        for row in range(NUM_ROWS):
            for col in range(NUM_COLS):
                ...
                if self.grid[row][col] == WALL:
                    color = "#424242"  # Dark grey for walls
                elif self.grid[row][col] == START:
                    color = "#66bb6a"  # Light green for start
                elif self.grid[row][col] == GOAL:
                    color = "#ff7043"  # Coral red for goal
                else:
                    color = "#ffffff"  # White for open space

                # Draw the grid cells with a new visual style
...
# Create and run the MazeSolverApp
